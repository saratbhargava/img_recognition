...
class ResNetBlock(nn.Module):
    def __init__(self, in_channels: int = 32):
        ...
        self.conv1 = nn.Conv2d(
            in_channels=in_channels, out_channels=num_filters, kernel_size=3, padding=1
        )
...
class ResNetChangeBlock(nn.Module):
    ...
    def forward(self, x):
        ...
        self.zero_identity = nn.Parameter(
            torch.zeros(
                n, self.out_channels - self.in_channels, int(w // 2), int(h // 2)
            ),
            requires_grad=False,
        )
        x_expand = F.pad(
            x[:, :, : int(w // 2), : int(h // 2)],
            pad=(0, 0, 0, 0, self.out_channels - self.in_channels, 0, 0, 0),
        )
